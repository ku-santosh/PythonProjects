...
from ..models.perspective import FilterDetail as FilterDetailModel
...
def save_single_filter_model_route():
    ...
    try:
        ...
        if existing_perspective:
            ...
            for validated_item in validated_filter_models:
                ...
                for i, existing_item in enumerate(existing_filter_model_list):
                    ...
                    if existing_item.name == validated_item_dict['name'] and existing_item.view == validated_item_dict[
                        'view']:
                        # --- FIX START ---
                        # Convert the raw dictionaries from Pydantic into `FilterDetailModel` objects
                        new_filters = {}
                        for k, v in validated_item_dict.get('filters', {}).items():
                            new_filters[k] = FilterDetailModel(**v)

                        # Update the existing DTO object's attributes with the corrected objects
                        existing_filter_model_list[i].filters = new_filters
                        # --- FIX END ---
            ...
            final_filter_model_list_for_pydantic = [fm.model_dump(mode='json') for fm in existing_filter_model_list]
            ...
            perspective_update = PerspectiveUpdate(
                ...
                column_state=[cs.model_dump() for cs in existing_perspective.column_state],
                sort_model=[sm.model_dump(mode='json') for sm in existing_perspective.sort_model],
