...
class FeedbackService:
    ...
    def get_feedback_by_id(self, feedback_id: int) -> Optional[Feedback]:
        ...
        return Feedback.from_dict(row) if row else None
    ...
    def create_or_append_feedback(self, feedback_in: FeedbackCreate) -> dict:
        ...
        feedbacks_json = json.dumps([f.model_dump() for f in feedback_in.user_feedbacks])
        ...
        if existing:
            ...
            row = self.db_curr.fetchone()
            self.db_conn.commit()
            feedback = Feedback.from_dict(row)
            return {**vars(feedback), "message": f"Feedback appended for user_gpn '{feedback_in.user_gpn}'"}
        else:
            self.db_curr.execute(
                """
            ...
            self.db_conn.commit()
            feedback = Feedback.from_dict(row)
            return {**vars(feedback), "message": f"user_gpn not found, created new record"}
    ...
    def update_feedback_by_gpn(self, user_gpn: str, feedback_in: FeedbackUpdate,
        ...
        feedbacks_json = json.dumps([f.model_dump() for f in feedback_in.user_feedbacks]) if feedback_in.user_feedbacks else "[]"
        ...
        if existing:
            ...
            self.db_conn.commit()
            feedback = Feedback.from_dict(row)
            return {**vars(feedback), "message": f"Feedback appended for user_gpn '{user_gpn}'"}
        else:
            self.db_curr.execute(
                """
                INSERT INTO recsui.recsFeedback (user_gpn, user_name, user_email, user_feedbacks, updated_time)
            ...
            self.db_conn.commit()
            feedback = Feedback.from_dict(row)
            return {**vars(feedback), "message": "user_gpn not found, created new record"}
